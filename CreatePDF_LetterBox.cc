...
int main(int argc, char *argv[]){
  ...
  const int MaxX   = args.bnds.x();
  const int MaxY   = args.bnds.y();
  const int MaxZ   = args.bnds.z();
  AxisGrid<int> agX({0, MaxX}, MaxX/10);
  AxisGrid<int> agY({0, MaxY}, MaxY/10);
  AxisGrid<int> agZ({0, MaxZ}, MaxZ/10);

  TrigTimePDFLetterBox TTPDF(agX.GetVCenters(), agY.GetVCenters(), agZ.GetVCenters());
