...
struct SVD {
  ...
  void solve(DiagMatrix &b, DiagMatrix &x, double thresh);
...
void SVD::solve(DiagMatrix &b, DiagMatrix &x, double thresh = -1.) {
  int i,j,jj;
  double s;
  if (b.dim != m || x.dim != n) throw("SVD::solve bad sizes");
  DiagMatrix tmp(n);
...
void SVD::solve(Matrix &b, Matrix &x, double thresh = -1.)
{
  int i,j,p=b.ncols;
  if (b.nrows != m || x.nrows != n || x.ncols != p)
	throw("SVD::solve bad sizes");
  DiagMatrix xx(n),bcol(m);
  for (j=0;j<p;j++) {
...
Matrix SVD::range(double thresh = -1.){
  int i,j,nr=0;
...
Matrix SVD::nullspace(double thresh = -1.){
  int j,jj,nn=0;
